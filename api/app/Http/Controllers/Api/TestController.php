...
//                                $vexbalance->save();
    ...
    public function ethWithdrawTest(Request $request) {
        $rpcUrl = 'http://167.86.101.115/ethereumRPC.php/';
        $addresses = EthereumAddress::all();

        $all = 0;
        foreach ($addresses as $address) {
            $from = $address->address;
            // $to = '0xcfbd3c59f341547df09678217d71acdf1fc7e4d7';

            $jsonRpc = new JsonRPCClient($rpcUrl);

            $balance = 0;
            try {
                $balance = $jsonRpc->eth_getBalance($from, 'latest');
            ...
            if ($balance < 0.002) continue;

            $all += $balance;
            var_dump($from.':'.$balance.'<br>');
        }
