...
@Component({
    templateUrl: './mpayico.component.html',
    styleUrls: ['./mpayico.component.scss']
})
export class MPAYICOComponent implements OnInit, AfterViewInit, OnDestroy {
    icoSetting: any = {};
    remain: any = {
        day: 0,
        hour: 0,
    ...
    ngOnInit() {
        this.icoApi.getMPAYSetting({}).subscribe(res => {
            if (res.success) {
    ...
    loadStatistics() {
        this.icoApi.getMPAYStatistics({}).subscribe(res => {
            if (res.success) {
    ...
    onClickBalance(coin) {
        if (this.selBuyCoin == coin) {
            ...
            if (coin == 'BTC') {
                this.mpayAmount = balance / this.btc_price;
            } else if (coin == 'ETH') {
    ...
    onBuyMPAY() {
        ...
        if (Number(this.mpayAmount) < Number(this.icoSetting.min_amount) || isNaN(this.mpayAmount)) {
            this.notify.showNotification('Warning', 'You cannot buy MPAY less than ' + this.icoSetting.min_amount + ' ', 'warning');
            return;
        }
        ...
        this.icoApi.buyMPAY({
            ...
            amount: this.mpayAmount,
            price: this.sel_price
        }).subscribe(res => {
            this.settings.loading = false;
            if (res.success) {
                this.notify.showNotification('Successfully', 'You bought MPAY successfully...',  'success');
                this.balance.getCoinBalance('MPAY');
